import { TerminalData, TokenInfo } from "@/types";
...
export function Tokenslist({
  ...
  useEffect(() => {
    const getTokenData = async () => {
      try {
        const data = (
          await apiFetcher(
            `https://api.coingecko.com/api/v3/coins/1/contract/${searchedToken}`
          )
        ).data as TerminalData;

        if (data) {
          const { name, symbol, image, detail_platforms } = data;
          const logoURI = image.small;
          const decimals = detail_platforms.ethereum.decimal_place;
  ...
  return (
    <div className="fixed h-full w-full bg-stone-900/70 backdrop-blur-md border-solid border-2 border-black rounded-lg flex items-center justify-center z-30">
